...
#if defined(NDEBUG)
# error "Pinkdog cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Pinkdog Signed Message:\n";
                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s PDG additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // 100001  - 500000    100
    // 500001  - 1000000   50
    // 1000001 - ..        Halving every 1 year

    if (nHeight <= 100    ) { return    1 * COIN; }
    if (nHeight <= 25000  ) { return 1000 * COIN; }
    if (nHeight <= 100000 ) { return  500 * COIN; }
    if (nHeight <= 500000 ) { return  100 * COIN; }
    if (nHeight <= 1000000) { return   50 * COIN; }

    int halvings = (nHeight - 1000000) / consensusParams.nSubsidyHalvingInterval;
    // Force block reward to zero when right shift is undefined.
    if (halvings >= 64)
        return 0;

    CAmount nSubsidy = 25 * COIN;
    // Subsidy is cut in half every 1100000 blocks which will occur approximately every year.
    nSubsidy >>= halvings;
    return nSubsidy;
}
